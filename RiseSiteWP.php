...
function rs_rise_post() {
	...
	if (isset($_POST['rs_scan'])) { 
					...
					<td colspan="5">
						<div class="submit">
							<input name="rs_url_page" type="hidden" value="<?=$_POST['rs_url_page']?>" />
					         	<input name="rs_rise" type="submit" class="button-primary" value="<?php echo __('Read Post'); ?>" />
					        </div>
					</td>
				</tr>
			</table>
		...
		echo "<textarea  name='rs_content' style='display: block;height: 400px;margin: 10px 0;width: 99%; border:1px solid #777;'>$html</textarea>"; 
        ...
        $rs_id = wp_insert_post( array(
  		...
  		'post_category' => $np['post_category'] //Добавление ID категорий.
            ...
            if ($rs_id) {wp_redirect("http://homeschoolmo.com/wp-admin/post.php?post=$rs_id&action=edit"); exit;} 
					...
					<table id='rs_list' cellspacing='0' style="margin: 0px auto;">
							...
							<tr>
								...
								<td>
									...
									(<a style='color:grey;' href="http://<?=$rs_domen_name.'/'.$key?>" target="_blank">На сайте</a>) 
...
function is_set_slug($value, $slug_array){
	...
	foreach ($slug_array as $slug_one) {
	 	if (!strpos ( $value, '/'.$slug_one.'.html' )===FALSE) return "color:green;";
	} 
	return;
}
